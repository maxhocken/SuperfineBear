...
%We will need to update the frame information eventually when we include
%time interval from metadata. 
...
[Perimlong, Perimshort] = autoCorrelate(frames,Perimdata);
title('Perimeter Periodicity')
%Cell velocity
Veldata = data(1:endframe,6);
figure

[Vellong, Velshort] = autoCorrelate(frames,Veldata);
title('Cell Velocity Periodicity')
