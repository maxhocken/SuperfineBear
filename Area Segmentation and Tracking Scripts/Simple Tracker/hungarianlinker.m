...
% implementing the Munkres algorithm; this file is merely a wrapper for it.
    ...
    % submission.
    [ assignment_matrix total_cost ] = munkres(D);
    %assignment_matrix has the same dimensions of D, with 1s indicating the
    %matches and zeroes everywhere else
    target_indices=NaN(n_source_points,1);
    %get target_indices from assignment_matrix
    for i = 1:size(assignment_matrix,1)
        %if an assignment was made
        if sum(assignment_matrix(i,:)) ~= 0
            target_indices(i)=find(assignment_matrix(i,:));
        else
            target_indices(i)=0;
        end
    end
